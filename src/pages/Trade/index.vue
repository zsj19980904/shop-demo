  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(item,index) in addressinfo" :key="item.id">
        ...
        <p @click="changeisDefault(item,addressinfo)">
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{userDefaultmind.fullAddress}}</span>
        收货人：<span>{{userDefaultmind.consignee}}</span>
        <span>{{userDefaultmind.phoneNum}}</span>
      </div>
    </div>
...
<script>
  ...
  export default {
	...
	computed:{
		...
		methods:{
			changeisDefault(address,addressinfo){
				//修改默认数据 
				addressinfo.forEach((itme,index)=>{
				 itme.isDefault=0;
				})
				address.isDefault=1;
			},
		async submitOrder(){
				...
				//data的六个参数
				let data={
				consignee:this.userDefaultmind.consignee,
				consigneeTel:this.userDefaultmind.phoneNum,
				deliveryAddress:this.userDefaultmind.userAddress,
				paymentWay:"ONLINE",
				orderComment:this.msg,
				orderDetailList:this.orderinfo.detailArrayList
				}
				let result = await this.$API.reqPay(tradeNo,data);
				if(result.code==200){
