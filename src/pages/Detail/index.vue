				...
				<div class="InfoWrap">
					...
					<div class="choose">
						<div class="chooseArea">
							...
							<dl v-for="(spu,index) in spuSaleAttrList" :key="spuSaleAttrList.id">
								<dt class="title">{{spu.saleAttrName}}</dt>
								<dd changepirce="0" :class="{active:value.isChecked==1}"
									v-for="(value,index) in spu.spuSaleAttrValueList"
									 :key="value.id"
									@click="chanagActive(value,spu.spuSaleAttrValueList)"
									 >
									{{value.saleAttrName}}</dd>
							</dl>
						</div>
...
<script>
	...
	export default {
		...
		methods:{
			...
			//这里的返回值也是一个promise 因为需要指定返回的结果是成功或者失败
		async addShopCar(){
			try{
			let result = await this.$store.dispatch('getAddOrUpdateShopCat',{skuId:this.$route.params.skuid,skuNum:this.skuNum})
            //路由跳转 路由跳转的时候 需要传递参数过去
			//比较简单的数据 如 skuNum,通过query形式给路由
			//产品信息的数据 如skuInfo 比较复杂 通过会话存储给路由 
			//使用本地存储来存数据并传递到购物车,JSON.stringify对象变成字符串
